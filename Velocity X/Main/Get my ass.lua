...
local DogSection = General:Section({ Title = "Dogs", Side = "l" })
...
DogSection:Button({
	Title = "Refresh Dogs",
	Callback = function()
		local newList = GetDogs()

		DogDropdown:Clear()
		for _, name in ipairs(newList) do
			DogDropdown:AddList(name)
		end

		getgenv().SelectedDog = nil

		Env.Notify({
			Title = "Dogs Refreshed",
			Desc = "Found " .. #newList .. " dog(s)",
			Type = "success",
			Duration = 2,
		})
	end,
...
task.spawn(function()
	while task.wait() do
		if not getgenv().AutoPetDog then continue end
